...
var findUsersinRoom = function(room, socket, callback){

	var users = [], temp = {}, current = 0;
	...
	room.connections.forEach(function(connection){
		...
		if(!temp[connection.userId]){
			users.push(connection.userId);
		}
		temp[connection.userId] = true;
	});
...
var disconnectUser = function(socket, callback){
